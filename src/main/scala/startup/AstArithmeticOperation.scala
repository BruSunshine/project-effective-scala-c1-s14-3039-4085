...
object ArithmeticOperation:
  ...
  /** Given instance of `ArithmeticOperation` for `Int`. Defines addition and
  ...
  /** Given instance of `ArithmeticOperation` for `Double`. Defines addition and
  ...
  /** Given instance of `ArithmeticOperation` for `DataFrame`. Defines addition
    * and multiplication for `DataFrame`. Addition concatenates string fields
    * and adds numeric fields. Multiplication concatenates string fields twice
    * and multiplies numeric fields.
    */
...
object OperationValidator:
  ...
  given OperationValidator[Dataset[Row]] with
    ...
    def validate(df1: Dataset[Row], df2: Dataset[Row], op: String): Boolean =
      op match
        case "add" | "mul" =>
          (df1.schema == df2.schema) && !df1.isEmpty && !df2.isEmpty
        case _ => false
